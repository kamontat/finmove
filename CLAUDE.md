...
1. **Title** — breadcrumb above the main box. Each screen owns its full title via `setTitle([...])` from `useLayout()`, with `return () => clearTitle()` in the cleanup. Falsy segments (`null`, `undefined`, `false`, `""`) are filtered, so `trip?.settings.name` can be spread directly without guarding loading states. Two helpers in `src/tui/utils/titles.ts`: `tripTitle(trip, ...rest)` produces `["Trips", trip?.settings.name, ...rest]` for trip-scoped screens; `settingsTitle(trip, ...rest)` extends that with `"Settings"` for trip-settings screens. Modal-style selectors (e.g. `OwnerSelect`) use bare `setTitle(["Select X"])` with no trip prefix. Screens with `if (!trip) return` early-returns in their menu/hints effect must keep title management in a SEPARATE dedicated `useEffect` so the cleanup always registers.
